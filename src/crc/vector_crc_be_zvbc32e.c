...
{
	int i;
  size_t avl = len / 4; // 8-byte per element
  ...
  for (; avl >= 2*vl; avl -= vl, p += 4 * vl, len -= 4*vl) {
      ...
      inputData = __riscv_vxor_vv_u32m1_tu(inputData, inputData, crcAcc, 4); // vl=4 since we only want to XOR the 128-bit crcAcc 
